...
import { ArrowLeft, Download, Loader2, Wind } from 'lucide-react';
...
const ARIView: React.FC = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const [data, setData] = useState<ARIRecord | null>(null);
        ...
        const fetchData = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    toast.error('Failed to fetch ARI notification');
                }
            } catch (err) {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            pdf.save(`ARI-Notification-${data.patientId || id}.pdf`);
    ...
    if (!data) return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50">
            <div className="text-center space-y-3">
                <Wind className="w-12 h-12 text-slate-300 mx-auto" />
                <p className="text-slate-500 font-medium">Record not found</p>
            </div>
        </div>
    );

    const fullName = [data.firstName, data.secondName].filter(Boolean).join(' ') || '-';
    const notifId = `ARI-${new Date(data.createdAt || Date.now()).getFullYear()}-${String(data.id).slice(-6).toUpperCase()}`;
    ...
    const SectionTitle = ({ children }: { children: React.ReactNode }) => (
        <h2 className="text-teal-800 font-bold uppercase mb-2 border-b-2 border-teal-200 pb-1 text-xs tracking-wider">{children}</h2>
    );
            ...
            <div className="max-w-5xl mx-auto space-y-4">
                ...
                <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-slate-200 print:hidden">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-lg hover:from-teal-700 hover:to-teal-800 transition-all font-medium shadow-md hover:shadow-lg transform hover:scale-105"
                    >
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-2xl font-bold text-slate-800 uppercase tracking-wide mb-1">
                                Acute Respiratory Illness (ARI) Case Notification Report
                            </h1>
                            <p className="text-slate-500 text-sm">Communicable Disease Surveillance Unit</p>
                            <div className="h-1 w-full bg-teal-700 mt-3 mb-0.5 rounded" />
                            <div className="h-0.5 w-full bg-teal-400 rounded" />
                        </div>

                        {/* Notification Details */}
                        <div className="mb-6 border border-slate-200 rounded">
                            <div className="bg-teal-700 text-white font-bold text-xs uppercase tracking-wider px-3 py-2 rounded-t">
                                Notification Details
                            </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="border border-slate-200 rounded divide-y divide-slate-200">
                                ...
                                <div className="grid grid-cols-2 divide-x divide-slate-200">
                                    ...
                                    <Row label="Next of Kin Mobile" value={val(data.nextOfKinMobileNo)} />
                                </div>
                                <div className="grid grid-cols-2 divide-x divide-slate-200">
                                    <Row label="Education"         value={val(data.education)} />
                                    <Row label="Work Status"       value={val(data.workStatus)} />
                                </div>
                                <div className="grid grid-cols-2 divide-x divide-slate-200">
                                    <Row label="Place of Work"     value={val(data.placeOfWork)} />
                                    <Row label="Passport No"       value={val(data.passportNo)} />
                                </div>
                                <div className="grid grid-cols-2 divide-x divide-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-5 mb-6">
                            ...
                            <div>
                                <SectionTitle>Section 2: Case & Vaccination Details</SectionTitle>
                                <div className="border border-slate-200 rounded divide-y divide-slate-200">
                                    <Row label="Case Type"         value={val(data.caseType)} />
                                    <Row label="Vaccination"       value={val(data.vaccinationDetails)} />
                                    <Row label="Sentinel Site"     value={val(data.sentinelSite)} />
                                    <Row label="Suspect Disease"   value={val(data.suspectDisease)} />
                                    <Row label="Disease Status"    value={val(data.diseaseStatus)} />
                                </div>
                            </div>
                            {/* Disease & Outcome */}
                            <div>
                                <SectionTitle>Section 3: Confirmed Disease & Outcome</SectionTitle>
                                <div className="border border-slate-200 rounded divide-y divide-slate-200">
                                    <Row label="Confirmed Disease" value={val(data.confirmedDiseases)} />
                                    <Row label="Source of Infection" value={val(data.sourceOfInfection)} />
                        ...
                        <div className="border-t border-slate-300 pt-4 mt-4">
                            <h3 className="font-bold text-teal-800 mb-2 text-sm uppercase">Declaration</h3>
                            <p className="text-sm text-slate-600">
                                This case has been notified as per Acute Respiratory Illness (ARI) disease surveillance and
                                reporting guidelines. All information contained herein is confidential and for official use only.
                            </p>
                        </div>
