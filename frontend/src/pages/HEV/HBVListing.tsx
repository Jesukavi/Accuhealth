...
const HBVListing: React.FC = () => {
    ...
    const fetchRecords = async (queryParams = new URLSearchParams()) => {
        ...
        try {
            ...
            const url = qs ? `${API_BASE_URL}/hbv-notifications?${qs}` : `${API_BASE_URL}/hbv-notifications`;
    ...
    const exportToExcel = async () => {
        try {
            ...
            XLSX.writeFile(wb, `HBV_Notifications_${dateStr}.xlsx`);
        } catch (error) {
                                    ...
                                    {filteredRecords.map((record) => (
                                        <tr key={record._id || record.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium">
                                                <button 
                                                  onClick={() => navigate(`/hbv-view/${record.id || record._id}`)}
                                                  className="text-blue-500 hover:text-blue-700 mr-3"
                                                >
                                                    View
                                                </button>
                                                <button
                                                  onClick={() => navigate(`/hbv-notification/${record.id || record._id}`)}
